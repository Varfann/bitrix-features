...
namespace EduCoder\Rest;

use \EduCoder\Rest\Exception;

/**
 * @class Rest
 *
 * Rest based on Pest (Pest is a REST client for PHP, @See http://github.com/educoder/pest for details).
 *
 * Was been modified by Panychev Dmitry <panychev@code-craft.ru>, Shershnev Roman <shershnev@code-craft.ru>, Code Craft Ltd.
 * Pest was modified for better PSR-4 compatibility and good exception model.
 *
...
class Rest
{
     ...
     */
    public $curl_opts = array(CURLOPT_RETURNTRANSFER => true,
    ...
    public function __construct($base_url) {
        ...
        $this->curl_opts[CURLOPT_HEADERFUNCTION] = array($this,
                                                         'handle_header');
    }
    ...
    protected function prepRequest($opts, $url) {
        ...
        $curl = curl_init($url);
        if ($curl === false) {
            throw new Exception\CurlInit($this->processError(curl_error($curl), 'curl'));
        }
        ...
        $this->last_request = array('url' => $url);
    ...
    protected function processError($body) {
        ...
        // of the resulting RestException
        return $body;
    }
     ...
     * @throws RestConflict
     * @throws RestGone
     * @throws RestUnauthorized
     * @throws RestClientError
     * @throws RestMethodNotAllowed
     * @throws RestNotFound
     * @throws RestBadRequest
     * @throws RestUnknownResponse
     * @throws RestInvalidRecord
     * @throws RestServerError
     * @throws RestForbidden
     */
    protected function checkLastResponseForError() {
        ...
        switch ($meta['http_code']) {
            case 400:
                throw new Exception\BadRequest($this->processError($body));
                break;
            case 401:
                throw new Exception\Unauthorized($this->processError($body));
                break;
            case 403:
                throw new Exception\Forbidden($this->processError($body));
                break;
            case 404:
                throw new Exception\NotFound($this->processError($body));
                break;
            case 405:
                throw new Exception\MethodNotAllowed($this->processError($body));
                break;
            case 409:
                throw new Exception\Conflict($this->processError($body));
                break;
            case 410:
                throw new Exception\Gone($this->processError($body));
                break;
            case 422:
                ...
                throw new Exception\InvalidRecord($this->processError($body));
                break;
            default:
                if ($meta['http_code'] >= 400 && $meta['http_code'] <= 499) {
                    throw new Exception\ClientError($this->processError($body));
                } elseif ($meta['http_code'] >= 500 && $meta['http_code'] <= 599) {
                    throw new Exception\ServerError($this->processError($body));
                } elseif (!isset($meta['http_code']) || $meta['http_code'] >= 600) {
                    throw new Exception\UnknownResponse($this->processError($body));
                }
        }
    }

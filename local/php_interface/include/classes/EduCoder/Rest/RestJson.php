...
class RestJson extends Rest
{
     ...
     *
     * @return string
     * @throws RestJsonEncode
     */
    public function jsonEncode($data) {
        $ret = json_encode($data);

        if ($ret === false && $this->throwJsonExceptions) {
            throw new Exception\JsonEncode('Encoding error: '
                                     . $this->getLastJsonErrorMessage(), $this->getLastJsonErrorCode());
        }
    ...
    public function jsonDecode($data, $asArray = true) {
        ...
        if ($ret === null && $this->hasJsonDecodeFailed() && $this->throwJsonExceptions) {
            throw new Exception\JsonDecode('Decoding error: '
                                     . $this->getLastJsonErrorMessage(), $this->getLastJsonErrorCode());
        }

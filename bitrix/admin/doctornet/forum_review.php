...
while ($res = $adminResult->NavNext(true, 'f_')) {
    ...
    if ((int)$res['PARAM2']) {
        $element               = CIBlockElement::GetList([], ['ID' => $res['PARAM2']], false, false, [
            'ID',
            'IBLOCK_ID',
            'IBLOCK_TYPE',
            'NAME',
        ...
        switch ($col['id']) {
            case 'AUTHOR_ID':
                if ($res['AUTHOR_ID'] && $user = CUser::GetByID($res['AUTHOR_ID'])->Fetch()) {
                    $row->AddViewField($col['id'],
                        '[<a href="user_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $res['AUTHOR_ID']
                        . '" target="_blank">' . $user['ID'] . '</a>]&nbsp;(' . htmlspecialcharsEx($user['LOGIN'])
    ...
    $actions = [
        [
            ...
            'ICON'    => 'edit',
            'TEXT'    => 'Редактировать',
            'ACTION'  => $lAdmin->ActionRedirect($APPLICATION->GetCurPageParam(bitrix_sessid_get() . '&ID[]=' . $f_ID
                                                                               . '&action_button=edit', [
                'sessid',
                'ID',
                'action_button',
